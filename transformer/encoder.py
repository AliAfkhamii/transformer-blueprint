...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim, model_dim, ff_dim, num_heads, pe_max_len=1000,
                 pe_dropout=0.1, att_dropout=0.1, att_norm_dropout=0.1, ff_norm_dropout=0.1, bias=False, num_layers=6):
        super().__init__()

        self.num_layers = num_layers

        self.embedding = Embedding(vocab_size, embed_dim, model_dim)
        self.PE = SinusoidalPE(model_dim, dropout=pe_dropout, max_len=pe_max_len)

        # construct num_layer encoder layers
        self.layers = nn.ModuleList([
            EncoderBlock(ff_dim, num_heads, model_dim, att_dropout, att_norm_dropout, ff_norm_dropout, bias)
